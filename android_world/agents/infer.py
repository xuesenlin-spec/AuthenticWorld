        ...
        while counter > 0:
            try:
                ...
                if response.ok and "choices" in response.json():
                    print(f"[TIMING] LLM HTTP request: {_llm_elapsed:.2f}s (status={response.status_code})")
                    return (
                ...
                if response.text:
                    ...
                    print(f"Error calling OpenAI API: empty response, status={response.status_code}")

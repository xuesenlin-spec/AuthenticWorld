...
PROMPT_PREFIX = (
    ...
    ' You can also use absolute coordinates: `{{"action_type": "click", "x": <x_pos>, "y": <y_pos>}}`\n'
    ...
    ' You can also use absolute coordinates: `{{"action_type": "long_press", "x": <x_pos>, "y": <y_pos>}}`\n'
    ...
    ' "index": <target_index>}}`'
    ' or use coordinates: `{{"action_type": "input_text", "text": <text_input>, "x": <x_pos>, "y": <y_pos>}}`\n'
    ...
    '--- Chained Actions ---\n'
    'If BOTH target UI elements are VISIBLE on the current screen (e.g., a text'
    ' input field AND a save button), you MAY output TWO consecutive actions in'
    ' one response to speed up execution. Use this for deterministic operations'
    ' where the second target is already visible. Format:\n'
    'Reason: ...\nAction 1: {{"action_type":...}}\nAction 2: {{"action_type":...}}\n\n'
    'Common patterns: input_text + click save, long_press + click Select All.'
    ' Do NOT chain actions where the second target only appears after the first.'
)
...
ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    'Reason: ...\nAction: {{"action_type":...}}\n'
    '(or Action 1: ...\nAction 2: ... if both targets are visible)\n\n'
    'Your Answer:\n'
)

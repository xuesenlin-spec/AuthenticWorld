...
def clear_directory(
    ...
    res = adb_utils.issue_generic_request(["shell", "ls", "-1", directory_path], env)
    folder_contents = res.generic.output.decode().replace("\r", "").strip()

    if folder_contents and "No such file" not in folder_contents:
        try:
            adb_utils.check_ok(
                adb_utils.issue_generic_request(
            ...
            # Ignore errors, just recreate the directory
            pass
...
def copy_dir(source_path: str, dest_path: str, env: env_interface.AndroidEnvInterface):
...
def check_directory_exists(path: str, env: env_interface.AndroidEnvInterface) -> bool:
...
def tmp_directory_from_device(
    ...
    logging.info("Copying %s directory to local tmp %s", device_path, tmp_directory)
...
def copy_file_to_device(
    ...
    with open(local_file_path, "rb") as f:
        ...
        push_request = adb_pb2.AdbRequest(
            push=adb_pb2.AdbRequest.Push(content=file_contents, path=remote_file_path),
            timeout_sec=timeout_sec,
        )

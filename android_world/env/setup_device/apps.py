...
def download_app_data(file_name: str) -> str:
    ...
    remote_url = f"https://storage.googleapis.com/gresearch/android_world/{file_name}"
...
class AppSetup(abc.ABC):
    ...
    def package_name(cls) -> str:
        return adb_utils.extract_package_name(adb_utils.get_adb_activity(cls.app_name))
    ...
    def setup(cls, env: interface.AsyncEnv) -> None:
        ...
        try:
            ...
            # Skip setup if app is not installed (e.g., Google-specific apps)
            print(f"Skipping setup for {cls.app_name}: {e}")
...
class CameraApp(AppSetup):
    ...
    def setup(cls, env: interface.AsyncEnv) -> None:
        ...
        adb_utils.grant_permissions(
            adb_utils.extract_package_name(adb_utils.get_adb_activity(cls.app_name)),

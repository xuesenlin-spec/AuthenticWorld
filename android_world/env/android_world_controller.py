...
def apply_a11y_forwarder_app_wrapper(
    ...
    # Check if forwarder app is already installed to avoid network download
    try:
        from android_env.components import adb_controller

        controller = env._coordinator._adb_call_parser._adb_controller
        result = controller.execute_command(
            ["shell", "pm", "path", "com.google.androidenv.accessibilityforwarder"]
        )
        if result.stdout and b"package:" in result.stdout:
            print("✅ Accessibility forwarder already installed, skipping download.")
            install_a11y_forwarding_app = False
    except Exception as e:  # pylint: disable=broad-exception-caught
        pass  # If check fails, proceed with original logic
